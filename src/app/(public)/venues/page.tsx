...
function VenuesSearchContent() {
  ...
  const didScrollToResultsRef = useRef(false);
  ...
  }, [debouncedKeyword, sportId, date, isOpenNow, nearMe, pageIndex, pageSize]);

  useEffect(() => {
    router.replace(`/venues?${queryString}${nearMe ? "#results" : ""}`, { scroll: false });
  }, [nearMe, queryString, router]);
  ...
  useEffect(() => {
    if (didScrollToResultsRef.current) return;
    if (!nearMe && window.location.hash !== "#results") return;
    didScrollToResultsRef.current = true;
    scrollToResults();
  }, [nearMe]);
  ...
  return (
    <div className="-mt-16 min-h-screen bg-[radial-gradient(circle_at_92%_8%,rgba(199,242,39,.22)_0,rgba(220,252,231,.42)_18%,transparent_36%),linear-gradient(135deg,#f7fff4_0%,#eefbea_38%,var(--pc-canvas)_78%)] pt-16">
      ...
      <main id="results" ref={resultsRef} className="mx-auto flex w-full max-w-7xl scroll-mt-20 flex-1 flex-col gap-6 px-6 py-14 lg:px-8">
